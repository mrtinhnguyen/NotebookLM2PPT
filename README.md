...
本工具的核心功能是通过模拟鼠标点击微软电脑管家的"转换为PPT"按钮来实现自动转换。因此，**按钮偏移参数决定了程序能否准确定位并点击该按钮**。

- **工作原理**：程序会自动识别智能圈选界面的"转换为PPT"按钮位置，并记录其相对于屏幕底部的垂直偏移量（像素值）。
- **首次使用**：建议勾选"校准按钮位置"，程序会引导你手动点击按钮位置，自动保存偏移值。
- **偏移值保存**：校准后的偏移值会自动保存到本地，下次使用时无需重复校准。
- **如果校准失败**：如果无法准确点击按钮，整个转换流程将无法进行。此时需要：
  - 确保微软电脑管家已更新到最新版本
  - 检查智能圈选功能是否正常工作
  - 尝试手动校准按钮位置
  - 清除保存的偏移值，重新进行校准
